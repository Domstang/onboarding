      ...
      <font-awesome-icon class="mr-1" icon="info-circle" />
        <i18n path="onboarding.see.documentation" tag="span">
            <template v-slot:link>
                <a target="_blank" href="https://docs.skalin.io/en/dev/tracking/">{{ $t('onboarding.see.documentation.label' )}}</a>
            </template>
        </i18n>
    </b-alert>
        ...
        <b-row>
          <b-col class="text-center">
            <button
              ...
              @click="goto()"
            >
              {{ $t("onboarding.tag.body.setup.tag") }}
            </button>
          </b-col>
        </b-row>
      </b-col>
...
export default {
  ...
  methods: {
    goto() {
      this.$router.push({ name: "integrationTracking" });
    },
    skipStep() {
      this.$emit("skip-step", this.step.key, "SKIPED");
      this.$root.$emit('bv::toggle::collapse', 'collapse-tracking')
    },
  },
};
</script>
